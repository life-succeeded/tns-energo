...
func (s *Service) Upload(ctx goctx.Context, _ golog.Logger, request UploadRequest) (file.File, error) {
	...
	name := fmt.Sprintf("%s_%s_%d.%s", request.Address, time.Now().In(gotime.Moscow).Format("02.01.2006_15.04"), imageNumbersCache[request.DeviceNumber], extension)

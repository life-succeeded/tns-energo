...
func NewRoute(log liblog.Logger, route *mux.Route, handler Handler, middlewares []Middleware) *Route {
...
func (r *Route) ServeHTTP(rs http.ResponseWriter, rq *http.Request) {
	var (
		...
		ctx     = NewContext(r.log, rsProxy, rq)
	)
	defer func() {
		if err := ctx.Close(); err != nil {
			r.log.Debugf("Не удалось закрыть swrouter.Context: %v", err)
		}
	}()
